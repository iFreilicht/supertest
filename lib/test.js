...
class Test extends Request {
  ...
  constructor (app, method, path) {
    ...
    this.end = (fn) => {
      // Ensure server is listening by waiting for the URL to be set
      let checkIfSet = () => {
        if (this.url === undefined) {
          setTimeout(checkIfSet, 10);
        } else {
          return;
        }
      };
    ...
    this.setUrl(app, path)
  }

  /**
   * Starts the server if it wasn't already and
   * sets the url attribute, once the server is running.
   *
   * @param {Server} app
   * @param {String} path
   * @api private
  ...
  setUrl(app, path) {
    ...
    if (app.address()) {
      this.url = this.constructUrl(app, path);
      return
    }

    this._server = app.listen({ port: 0, host: '127.0.0.1'}, () => {
      // Set URL only after the server is listening. This can be used later to ensure the server is listening
      this.url = this.constructUrl(app, path);
    });
  }

  /**
   * Constructs the URL of this test.
   *
   * @param {Server} app
   * @param {String} path
   * @api private
   */
  constructUrl(app, path) {
